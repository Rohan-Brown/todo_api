...
def test_update_and_delete_task(token_headers: dict, client: TestClient):
    ...
    resp = client.post(
        "/tasks",
        json={"title": "Created", "description": "my_desciption"},
...
def test_cannot_get_other_users_task(
    token_headers: dict, other_token_headers: dict, client: TestClient
):
...
def test_cannot_update_other_users_task(
    ...
    resp2 = client.put(
        f"/tasks/{tid}",
        json={"title": "You have been hacked"},
        headers=other_token_headers,
    )
    assert resp2.status_code == 403
...
def test_not_found_vs_forbidden(
    token_headers: dict, other_token_headers: dict, client: TestClient
):
...
def test_public_list_multiple_users(
    token_headers: dict, other_token_headers: dict, client: TestClient
):
...
def test_update_nonexistent_task_returns_404(token_headers: dict, client: TestClient):
    ...
    resp = client.put(
        "/tasks/9999", json={"title": "DifferentTitle"}, headers=token_headers
    )
    assert resp.status_code == 404
    assert resp.json()["detail"] == "Task not found"
...
def test_mark_completed_twice(token_headers: dict, client: TestClient):
    ...
    resp = client.post(
        "/tasks", json={"title": "completed_twice"}, headers=token_headers
    )
    tid = resp.json()["id"]
...
def test_filter_by_status_endpoint(token_headers: dict, client: TestClient):
    ...
    resp = client.get(
        "/tasks/filter-by-status/?status=Completed", headers=token_headers
    )
    assert resp.status_code == 200
...
def test_filter_and_pagination(token_headers: dict, client: TestClient):
    ...
    for j in range(2):
        t = client.post(
            "/tasks", json={"title": f"Comp{j}"}, headers=token_headers
        ).json()
        completed_ids.append(t["id"])
        client.put(f"/tasks/{t['id']}/complete", headers=token_headers)

    resp = client.get("/tasks?status=Completed&skip=1&limit=1", headers=token_headers)
    data = resp.json()
...
def test_public_filter_by_status(token_headers: dict, client: TestClient):
    ...
    t = client.post(
        "/tasks", json={"title": "Should be completed"}, headers=token_headers
    ).json()
    client.put(f"/tasks/{t['id']}/complete", headers=token_headers)
    ...
    assert resp.status_code == 200
    assert all(
        task["status"] == TaskStatus.completed.value for task in resp.json()["tasks"]
    )
...
def test_cascade_delete_user(db_session):
    ...
    u = User(
        first_name="DeleteFName",
        last_name="DeleteLName",
    ...
    assert (
        deleted_task is None
    ), "Task should be deleted after user deletion due to cascade"
...
def test_filter_by_status_pagination(token_headers, client):
    ...
    resp = client.get(
        "/tasks/filter-by-status/?status=Completed&skip=0&limit=3",
        headers=token_headers,
    )
    data = resp.json()
    assert resp.status_code == 200
...
def test_filter_by_status_invalid_status(token_headers, client):
    ...
    resp = client.get(
        "/tasks/filter-by-status/?status=InvalidStatus", headers=token_headers
    )
    assert resp.status_code == 422
