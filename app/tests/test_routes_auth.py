...
def test_register_and_login(
    client: TestClient,
):  # Happy path testing for register and login
    register_payload = {
    ...
    register_response = client.post(
        "/auth/register", json=register_payload
    )  # Register user
...
def test_register_duplicate_username(
    client: TestClient,
):  # Testing if duplicates can be made
    user_data = {
    ...
    )  # registering the same information again
...
def test_login_invalid_credentials(
    client: TestClient,
):  # Checking if login function with incorrect credentials
    resp1 = client.post(
...
def test_register_without_last_name(
    client: TestClient,
):  # Testing if you can register without inputing a last name
    payload = {
        "first_name": "first_no_last",
        "username": "no_last",
        "password": "no_last",
    }
...
def test_register_short_password(
    client: TestClient,
):  # Minimum 6 characters for password enforcement
